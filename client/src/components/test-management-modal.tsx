...
export default function TestManagementModal({ isOpen, onOpenChange }: TestManagementModalProps) {
  ...
  const { data: categoriesData } = useQuery({
    ...
    queryFn: async () => {
      const data = await apiClient.get("/api/test-categories");
  ...
  const { data: questionsData } = useQuery({
    ...
    queryFn: async () => {
      const data = await apiClient.get("/api/test-questions");
  ...
  const { data: resultsData } = useQuery({
    ...
    queryFn: async () => {
      const data = await apiClient.get("/api/test-results");
      return data.results || [];
    }
  });
  ...
  const deleteCategoryMutation = useMutation({
    mutationFn: async (id: number) => {
      return await apiClient.delete(`/api/test-categories/${id}`);
    },
    onSuccess: () => {
  ...
  const deleteQuestionMutation = useMutation({
    mutationFn: async (id: number) => {
      return await apiClient.delete(`/api/test-questions/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/test-questions'] });
      toast({
  ...
  const deleteResultMutation = useMutation({
    mutationFn: async (id: number) => {
      return await apiClient.delete(`/api/test-results/${id}`);
    },
    onSuccess: () => {
